...
public partial class MainWindow : Window
{
    ...
    private async void btnEncrypt_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (file == null)
            {
                box = MessageBoxManager
                    .GetMessageBoxStandard("Caption", "Encryption cancelled.",
                        ButtonEnum.Ok);
                await box.ShowAsync();
                return;
            }
            ...
            if (!isFileInput) // Text input
            {
                ...
                {
                    box = MessageBoxManager
                    .GetMessageBoxStandard("Caption", "No text input provided.",
                        ButtonEnum.Ok);
                    await box.ShowAsync();
                    return;
                }
            ...
            box = MessageBoxManager
                  .GetMessageBoxStandard("Caption", "Encrypted file saved successfully.",
                            ButtonEnum.Ok);
            await box.ShowAsync();
        }
        catch (Exception ex)
        {
            box = MessageBoxManager
                  .GetMessageBoxStandard("Caption", $"Encryption error: {ex.Message}",
                            ButtonEnum.Ok);
            await box.ShowAsync();
        }
    }
    ...
    private async void btnDecrypt_Click(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                box = MessageBoxManager
                  .GetMessageBoxStandard("Caption", "File not found.",
                            ButtonEnum.Ok);
                await box.ShowAsync();
                return;
            }
            ...
            {
                box = MessageBoxManager
                  .GetMessageBoxStandard("Caption", "Please enter the decryption key.",
                            ButtonEnum.Ok);
                await box.ShowAsync();
                return;
            }
            ...
            {
                box = MessageBoxManager
                  .GetMessageBoxStandard("Caption", $"Error: Selected method '{method}' does not match file extension '{extension}'.",
                            ButtonEnum.Ok);
                await box.ShowAsync();
            ...
            {
                box = MessageBoxManager
                            .GetMessageBoxStandard("Caption", $"Decryption cancelled.",
                            ButtonEnum.Ok);
                await box.ShowAsync();
                return;
            } else {
            ...
            else
            {
                ...
                if (fileMethod != method)
                {
                    ...
                    box = MessageBoxManager
                            .GetMessageBoxStandard("Caption", $"Error: File contents indicate a different encryption method than selected.",
                            ButtonEnum.Ok);
                    await box.ShowAsync();
                    return;
                }
            ...
            box = MessageBoxManager
                    .GetMessageBoxStandard("Caption", "Decryption successful! File saved.",
                    ButtonEnum.Ok);
            await box.ShowAsync();
        ...
        {
            box = MessageBoxManager
                    .GetMessageBoxStandard("Caption", $"Decryption error: {ex.Message}",
                    ButtonEnum.Ok);
            await box.ShowAsync();
        }
    }
